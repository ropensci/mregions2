...
The Marine Regions Gazetteer is accessible as Linked Data Event Streams. This is described with high detail in: 
...
HTTP requests are handled with the R package [httr2](https://httr2.r-lib.org/)

```{r chunk1}
library(mregions2)

bpns <- gaz_rest_record_by_mrgid(3293, rdf = TRUE)

bpns

```

The MRGID includes info about the Gazetteer record and its hierarchy. The relations are explained the Marine Regions Ontology:
...
You can then use the [rdflib](https://docs.ropensci.org/rdflib/) R package further to apply common RDF applications. 

E.g. you can serialize the record as JSON-LD
```{r chunk2}
library(rdflib)

# doc = rdf()

# rdf_serialize(bpns, doc = doc, format = "jsonld")
