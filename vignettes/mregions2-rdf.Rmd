...
The Marine Regions Gazetteer is accessible as Open Linked Data, and can be mirrored and synchronized via Linked Data Event Streams. This is described with high detail in: 
...
Known an [MRGID][mregions2], you can get it as RDF. 
...
2. Read this document as RDF using the [rdflib](https://docs.ropensci.org/rdflib/) R package. 

HTTP requests are handled with [httr2](https://httr2.r-lib.org/)
...
You can then use the [rdflib](https://docs.ropensci.org/rdflib/) R package to apply common RDF applications. 
...
readLines(bpns_as_jsonld, n = 13) %>% cat()
```
...
The relationships are contained in the Marine Regions Ontology at <http://marineregions.org/ns/ontology>. The following example extracts all the predicates with the prefix `mr:`, pointing to the ontology:
...
This URI can be requested as Turtle or JSON-LD and load as an [rdf] object of the [rdflib] R package. 
...
As described in [Loneville B. et al. (2021)](http://ceur-ws.org/Vol-2969/paper8-s4biodiv.pdf), Marine Regions offers a Linked Data Event Stream (LDES) that can be used for replication and synchronization.

Building such API in R is, at the moment, out of the scope of mregions2. An interested user could however make use of mregions2 and rdflib to mirror the Marine Regions Gazetteer. 
...
rdflib allows to store the triples in a database. This is further explained in the vignette [Working with Database Storage Backends in `rdflib`](https://docs.ropensci.org/rdflib/articles/storage.html)
...
This feed can be paginated by time stamps. This allows to see if some gazetteer entries have changed over time. Eventually, if the records included in your local graph of the Marine Regions Gazetter are outdated, you can write a service that checks if the record has been modified and synchronize with your local graph.
